...
void MONITOR2_JSON(AsyncWebServerRequest *request)
{
    ...
    */
    ...
    for (int i = 0; i < _mysettings->totalControllers; i++)
    {
        ...
        PrintStreamComma(response, "\"banks\":", _mysettings->totalNumberOfBanks[i]);
        PrintStreamComma(response, "\"seriesmodules\":", _mysettings->totalNumberOfSeriesModules[i]);
        //  PrintStreamComma(response, "\"received\":", _mysettings->pac);
        ...
        for (uint8_t j = 0; j < totalModules; j++)
        {
            ...
            {
                response->print(CMI_Get(i, j)->voltagemV);
            }
            else
            {
        ...
        for (uint8_t j = 0; j < totalModules; j++)
        {
            ...
            if (CMI_Get(i, j)->valid)
            {
                response->print(CMI_Get(i, j)->voltagemVMin);
            }
            else
            {
                // Module is not yet valid so return null values...
        ...
        for (uint8_t j = 0; j < totalModules; j++)
        {
            ...
            if (CMI_Get(i, j)->valid)
            {
                response->print(CMI_Get(i, j)->voltagemVMax);
            }
            else
            {
                // Module is not yet valid so return null values...
        ...
        for (uint8_t j = 0; j < totalModules; j++)
        {
            ...
            if (CMI_Get(i, j)->valid && CMI_Get(i, j)->internalTemp != -40)
            {
                response->print(CMI_Get(i, j)->internalTemp);
            }
            else
            {
                // Module is not yet valid so return null values...
        ...
        for (uint8_t j = 0; j < totalModules; j++)
        {
            ...
            if (CMI_Get(i, j)->valid && CMI_Get(i, j)->internalTemp != -40)
            {
                response->print(CMI_Get(i, j)->externalTemp);
            }
            else
            {
                // Module is not yet valid so return null values...
        ...
        for (uint8_t j = 0; j < totalModules; j++)
        {
            ...
            if (CMI_Get(i, j)->valid && CMI_Get(i, j)->inBypass)
            {
                response->print('1');
            }
            else
        ...
        for (uint8_t j = 0; j < totalModules; j++)
        {
            ...
            if (CMI_Get(i, j)->valid && CMI_Get(i, j)->inBypass)
            {
                response->print(CMI_Get(i, j)->PWMValue);
            }
            else
            {
                response->print('0');
        ...
        for (uint8_t j = 0; j < totalModules; j++)
        {
            ...
            if (CMI_Get(i, j)->valid && CMI_Get(i, j)->bypassOverTemp)
            {
                response->print('1');
            }
            else
        ...
        if (i + 1 < _mysettings->totalControllers)
        {
            response->print(comma);
        }
    }
